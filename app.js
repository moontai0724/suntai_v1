...
async function MessageHandler(event) {
	...
	switch (event.type) {
		case 'message':
			if (!(event.message.type == 'text' && event.message.text.startsWith('//') && authorize == true)) Chatlog.log(event);
									...
									switch (msgs[1]) {
										...
										case 'send':
											if (msgs[2] && msgs[3]) {
												if (msgs[2] == 'AllGroup') {
													db_settings.all('SELECT * FROM Groups').then(data => {
														...
														for (let i = 0; i < data.length; i++) {
															LineBotClient.pushMessage(encodeURI(data[i].id), MsgFormat.Text(event.message.text.replace('/mt send ' + msgs[2] + ' ', '')));
														}
														startReply(MsgFormat.Text('目前有： ' + reply));
													});
												} else if (msgs[2] == 'AllRoom') {
													db_settings.all('SELECT * FROM Rooms').then(data => {
														let reply = data[0].id;
														for (let i = 0; i < data.length; i++) {
															LineBotClient.pushMessage(encodeURI(data[i].id), MsgFormat.Text(event.message.text.replace('/mt send ' + msgs[2] + ' ', '')));
												...
												} else LineBotClient.pushMessage(encodeURI(msgs[2]), MsgFormat.Text(event.message.text.replace('/mt send ' + msgs[2] + ' ', '')));
										...
										if (msgs[2] == 'help') {
											...
											for (let i = 3; i < msgs.length; i = i + 2) {
												if (msgs[i] && msgs[i + 1]) {
													...
													if (i >= msgs.length - 2) {
														Chatlog.searchHistory(SourceData, Number(msgs[2]), settings, changelog).then(data => startReply(MsgFormat.Text(data.length > 2000 ? '內容過長，請縮小範圍後重新嘗試。' : data)));
													}
												} else {
													startReply(MsgFormat.Text('所賦予的參數有錯誤。'));
													break;
											...
											Chatlog.searchHistory(SourceData, 5, settings, changelog).then(data => {
												startReply(MsgFormat.Text(data.length > 2000 ? '內容過長，請縮小範圍後重新嘗試。' : data));
											});
										}
										break;
									case 'fortune': case 'f':
							...
							AllCity.forEach(element => {
								if (msg.includes(element)) {
									Weather.getCityWeather(element).then(weather => {
										AirQuality.get(element).then(AirQualityNow => {
											Ultraviolet.get(element).then(UltravioletNow => {
												startReply(MsgFormat.Text(weather),
													MsgFormat.Text('目前觀測：' + AirQualityNow + UltravioletNow));
											});
										});
									});
								}
...
setTimeout(function checkConnect(ms = 10000) {
	setTimeout(function () {
		...
		UTC8Time.getNowTimePromise().then(time => checkConnect((((30 - time.time_min % 30) * 60) - time.time_sec) * 1000 - time.time_ms + 5000));
	}, ms);
});
