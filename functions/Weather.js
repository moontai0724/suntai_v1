...
function getCityWeather(city) {
    getAllWeather().then(data => {
        ...
        let weatherMsg = '地區 ' + weather.areaName + ' 於 ' + today + ' 的天氣概況如下：';
        for (let i = 0; i < 3; i++) {
            weatherMsg += '\n' + weather.date[i] + ' ' + weather.time[i] + ' ' + weather.time[(i < 2) ? i + 1 : 0] +
                ': \n　降雨機率：' + weather.rainfall[i] +
                '\n　氣溫：' + weather.minTamperature[i] + ' 至 ' + weather.maxTamperature[i] +
                '\n　天氣 ' + weather.weatherDescription[i] + '\n　氣溫 ' + weather.tamperatureDestription[i];
        }
    });
}
            ...
            success: function (data) {
                ...
                for (let i = 0; i < data.records.location.length; i++) {
                    weather[weather.length] = {
                        ...
                        date: [
                            data.records.location[i].weatherElement[0].time[0].startTime.split(' ')[0].replace('-', '/').split('/')[1].replace('-', '/'),
                            data.records.location[i].weatherElement[0].time[1].startTime.split(' ')[0].replace('-', '/').split('/')[1].replace('-', '/'),
                            data.records.location[i].weatherElement[0].time[2].startTime.split(' ')[0].replace('-', '/').split('/')[1].replace('-', '/')
                        ],
                        time: [
                            data.records.location[i].weatherElement[0].time[0].startTime.split(' ')[1].split(':')[0],
                            data.records.location[i].weatherElement[0].time[1].startTime.split(' ')[1].split(':')[0],
                            data.records.location[i].weatherElement[0].time[2].startTime.split(' ')[1].split(':')[0]
                        ],
                        weatherDescription: [
